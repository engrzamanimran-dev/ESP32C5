#include "esp_event.h"
#include "esp_wifi.h"
#include "esp_log.h"
#include "esp_private/wifi.h"
#include "esp_wifi_netif.h"
#include "wifi_sta.h"

#define LOG_MAXIMUM_LEVEL 3

static const char *TAG = "wifi_sta";

static esp_netif_t *s_wifi_netif = NULL;
static EventGroupHandle_t s_wifi_event_group = NULL;
static wifi_netif_driver_t s_wifi_driver = NULL;

static void on_wifi_event(void *arg,esp_event_base_t event_base, int32_t event_id, void* event_data){
    switch (event_id)
    {
    case WIFI_EVENT_STA_START:
        if(s_wifi_netif != NULL) {
            esp_wifi_connect();
        }
        break;
    case WIFI_EVENT_STA_STOP:
        if(s_wifi_netif != NULL) {
            esp_netif_action_stop(s_wifi_netif, event_base, event_id, event_data);
        }
        break;
    case WIFI_EVENT_STA_CONNECTED:
        if(s_wifi_netif == NULL){
            ESP_LOGE(TAG, "WiFi STA netif is NULL");
            return;
        }

        wifi_event_sta_connected_t* event_sta_connected = (wifi_event_sta_connected_t*) event_data;
        ESP_LOGI(TAG, "Connected to SSID:%.*s, BSSID:%02x:%02x:%02x:%02x:%02x:%02x, Channel:%d, Authmode:%d, AID:%d",
                 event_sta_connected->ssid_len,
                 event_sta_connected->ssid,
                 event_sta_connected->bssid[0],
                 event_sta_connected->bssid[1],
                 event_sta_connected->bssid[2],
                 event_sta_connected->bssid[3],
                 event_sta_connected->bssid[4],
                 event_sta_connected->bssid[5],
                 event_sta_connected->channel,
                 event_sta_connected->authmode,
                 event_sta_connected->aid);

        wifi_netif_driver_t driver = esp_netif_get_io_driver(s_wifi_netif);
        if(!esp_wifi_is_if_ready_when_started(driver)){
            esp_err_t ret = esp_wifi_register_if_rxcb(driver, esp_netif_receive, s_wifi_netif);
            if(ret != ESP_OK){
                ESP_LOGE(TAG, "Failed to register rx callback");
                return;
            }
        }

        esp_netif_action_connected(s_wifi_netif, event_base, event_id, event_data);
        xEventGroupSetBits(s_wifi_event_group, WIFI_STA_CONNECTED_BIT);
        break;    
    case WIFI_EVENT_STA_DISCONNECTED:
        if(s_wifi_netif != NULL) {
            esp_netif_action_disconnected(s_wifi_netif, event_base, event_id, event_data);
        }
        xEventGroupClearBits(s_wifi_event_group, WIFI_STA_CONNECTED_BIT);
#if CONFIG_WIFI_STA_AUTO_RECONNECT
        ESP_LOGI(TAG, "Reconnecting to the AP...");
        wifi_sta_reconnect();
#endif

    default:
        break;
    }
}

static void on_ip_event(void* arg, esp_event_base_t event_base,
                        int32_t event_id, void* event_data) {

    esp_err_t ret;
    switch (event_id)
    {
#if CONFIG_WIFI_STA_CONNECT_IPV4 || CONFIG_WIFI_STA_CONNECT_UNSPECIFIED
    case IP_EVENT_STA_GOT_IP:
        if(s_wifi_netif == NULL){
            ESP_LOGE(TAG, "On IPv4 got IP, Interface handel is NULL");
            return;
        }
        ret = esp_wifi_internal_set_sta_ip();
        if(ret != ESP_OK){
            ESP_LOGE(TAG, "Failed to notify WiFi driver about got IPv4 address");
        }

        xEventGroupSetBits(s_wifi_event_group, WIFI_STA_IPV4_OBTAINED_BIT);

        ip_event_got_ip_t *event_ip = (ip_event_got_ip_t *) event_data;
        esp_netif_ip_info_t *ip_info = &event_ip->ip_info;
        ESP_LOGI(TAG, "IPv4 address: " IPSTR, IP2STR(&ip_info->ip));
        ESP_LOGI(TAG, "Subnet mask: " IPSTR, IP2STR(&ip_info->netmask));
        ESP_LOGI(TAG, "Gateway address: " IPSTR, IP2STR(&ip_info->gw));

        /* code */
        break;
#endif

#if CONFIG_WIFI_STA_CONNECT_IPV6 || CONFIG_WIFI_STA_CONNECT_UNSPECIFIED
            case IP_EVENT_GOT_IP6:
                if(s_wifi_netif == NULL){
                    ESP_LOGE(TAG, "On IPv6 got IP, Interface handel is NULL");
                    return;
                }

                ret = esp_wifi_internal_set_sta_ip();
                if (ret != ESP_OK){
                    ESP_LOGE(TAG, "Failed to notify WiFi driver about got IPv6 address");
                }

                xEventGroupSetBits(s_wifi_event_group, WIFI_STA_IPV6_OBTAINED_BIT);
                ip_event_got_ip6_t *event_ip6 = (ip_event_got_ip6_t *) event_data;
                esp_netif_ip6_info_t *ip6_info = &event_ip6->ip6_info;
                ESP_LOGI(TAG, "IPv6 address: " IPV6STR, IPV62STR(ip6_info->ip));
                ESP_LOGI(TAF, "Netmask: " IPV6STR, IPV62STR(ip6_info->netmask));
                ESP_LOGI(TAG, "Gateway address: " IPV6STR, IPV62STR(ip6_info->gw));

#endif
                case IP_EVENT_STA_LOST_IP:
                    xEventGroupClearBits(s_wifi_event_group, WIFI_STA_IPV4_OBTAINED_BIT);
                    xEventGroupClearBits(s_wifi_event_group, WIFI_STA_IPV6_OBTAINED_BIT);
                    ESP_LOGI(TAG, "Lost IP address");
    default:
        ESP_LOGI(TAG, "Unhandled IP event: %d", event_id);
        break;
    }
}

static void wifi_start(void *esp_netif, esp_event_base_t event_base,
                       int32_t event_id, void *event_data) {
    uint8_t mac_addr[6]={0};
    esp_err_t ret;

    wifi_netif_driver_t driver = esp_netif_get_io_driver(esp_netif);
    if(driver == NULL){
        ESP_LOGE(TAG, "Failed to get WiFi driver handle from netif");
        return;
    }

    ret = esp_wifi_get_if_mac(driver, mac_addr);
    if(ret != ESP_OK){
        ESP_LOGE(TAG, "Failed to get MAC address");
        return;
    }

    ESP_LOGI(TAG, "WiFi STA MAC address: %02x:%02x:%02x:%02x:%02x:%02x",
             mac_addr[0], mac_addr[1], mac_addr[2],
             mac_addr[3], mac_addr[4], mac_addr[5]);
    
    ret = esp_wifi_internal_reg_netstack_buf_cb(esp_netif_netstack_buf_ref, esp_netif_netstack_buf_free);
    if(ret != ESP_OK){
        ESP_LOGE(TAG, "Error (%d): Netstack callback registration failed", ret );
    }

    ret = esp_netif_set_mac(esp_netif, mac_addr);
    if(ret != ESP_OK){
        ESP_LOGE(TAG, "Failed to set MAC address");
    }

    esp_netif_action_start(esp_netif,event_base,event_id,event_data);

    ESP_LOGI(TAG, "Connecting to %s...", CONFIG_WIFI_STA_SSID);    
    ret = esp_wifi_connect();
    if(ret!= ESP_OK){
        ESP_LOGE(TAG, "Failed to connect to WiFi");
    }
}

esp_err_t wifi_sta_init(EventGroupHandle_t wifi_event_group) {
    esp_err_t ret;

    if(wifi_event_group != NULL){
        s_wifi_event_group = wifi_event_group;
    } else {
        ESP_LOGE(TAG, "WiFi event group handle is NULL");
        return ESP_FAIL;
    }

    esp_netif_config_t netif_config = ESP_NETIF_DEFAULT_WIFI_STA();
    s_wifi_netif = esp_netif_new(&netif_config);
    if(s_wifi_netif == NULL){
        ESP_LOGE(TAG, "Failed to create WiFi STA netif");
        return ESP_FAIL;
    }

    s_wifi_driver = esp_wifi_create_if_driver(WIFI_IF_STA);
    if(s_wifi_driver == NULL){
        ESP_LOGE(TAG, "Failed to create WiFi STA driver");
        return ESP_FAIL;
    }

    ret = esp_netif_attach(s_wifi_netif, s_wifi_driver);
    if(ret != ESP_OK){
        ESP_LOGE(TAG, "Failed to attach WiFi driver to netif");
        return ESP_FAIL;
    }

    ret = esp_event_handler_register(WIFI_EVENT,
                                              WIFI_EVENT_STA_START,
                                              &on_wifi_event,
                                              NULL);
    if(ret != ESP_OK){
        ESP_LOGE(TAG, "Failed to register WiFi start event handler");
        return ESP_FAIL;

    }
    ret = esp_event_handler_register(WIFI_EVENT,
                                              WIFI_EVENT_STA_STOP,
                                              &on_wifi_event,
                                              NULL);
    if(ret != ESP_OK){
        ESP_LOGE(TAG, "Failed to register WiFi stop event handler");
        return ESP_FAIL;

    }

    ret = esp_event_handler_register(WIFI_EVENT,
                                              WIFI_EVENT_STA_CONNECTED,
                                              &on_wifi_event,
                                              NULL);
    if(ret != ESP_OK){
        ESP_LOGE(TAG, "Failed to register WiFi connected event handler");
        return ESP_FAIL;

    }

    ret = esp_event_handler_register(WIFI_EVENT,
                                              WIFI_EVENT_STA_DISCONNECTED,
                                              &on_wifi_event,
                                              NULL);
    if(ret != ESP_OK){
        ESP_LOGE(TAG, "Failed to register WiFi disconnected event handler");
        return ESP_FAIL;

    }
#if CONFIG_WIFI_STA_CONNECT_IPV4 || CONFIG_WIFI_STA_CONNECT_UNSPECIFIED
    ret = esp_event_handler_register(IP_EVENT,
                                              IP_EVENT_STA_GOT_IP,
                                              &on_ip_event,
                                              NULL);
    if(ret != ESP_OK){
        ESP_LOGE(TAG, "Failed to register IP got IPv4 event handler");
        return ESP_FAIL;

    }
#endif

#if CONFIG_WIFI_STA_CONNECT_IPV6 || CONFIG_WIFI_STA_CONNECT_UNSPECIFIED
    ret = esp_event_handler_register(IP_EVENT,
                                              IP_EVENT_STA_GOT_IP6,
                                              &on_ip_event,
                                              NULL);
    if(ret != ESP_OK){
        ESP_LOGE(TAG, "Failed to register IP got IPv6 event handler");
        return ESP_FAIL;

    }
#endif
    ret = esp_event_handler_register(IP_EVENT,
                                              IP_EVENT_STA_LOST_IP,
                                              &on_ip_event,
                                              NULL);
    if(ret != ESP_OK){
        ESP_LOGE(TAG, "Failed to register IP lost IP event handler");
        return ESP_FAIL;

    }

    ret = esp_register_shutdown_handler((shutdown_handler_t) esp_wifi_stop);
    if(ret != ESP_OK){
        ESP_LOGE(TAG, "Failed to register WiFi shutdown handler");
        return ESP_FAIL;
    }

    wifi_init_config_t cfg = WIFI_INIT_CONFIG_DEFAULT();
    ret = esp_wifi_init(&cfg);
    if(ret != ESP_OK){
        ESP_LOGE(TAG, "Failed to initialize WiFi");
        return ESP_FAIL;
    }
    ret = esp_wifi_set_mode(WIFI_MODE_STA);
    if(ret != ESP_OK){
        ESP_LOGE(TAG, "Failed to set WiFi mode to STA");
        return ESP_FAIL;
    }

#if CONFIG_WIFI_STA_AUTH_OPEN
    const wifi_auth_mode_t auth_mode = WIFI_AUTH_OPEN;
#elif CONFIG_WIFI_STA_AUTH_WEP
    const wifi_auth_mode_t auth_mode = WIFI_AUTH_WEP;
#elif CONFIG_WIFI_STA_AUTH_WPA_PSK
    const wifi_auth_mode_t auth_mode = WIFI_AUTH_WPA_PSK;
#elif CONFIG_WIFI_STA_AUTH_WPA2_PSK
    const wifi_auth_mode_t auth_mode = WIFI_AUTH_WPA2_PSK;
#elif CONFIG_WIFI_STA_AUTH_WPA_WPA2_PSK
    const wifi_auth_mode_t auth_mode = WIFI_AUTH_WPA_WPA2_PSK;
#elif CONFIG_WIFI_STA_AUTH_WPA3_PSK
    const wifi_auth_mode_t auth_mode = WIFI_AUTH_WPA3_PSK;
#elif CONFIG_WIFI_STA_AUTH_WPA2_WPA3_PSK
    const wifi_auth_mode_t auth_mode = WIFI_AUTH_WPA2_WPA3_PSK;
#elif CONFIG_WIFI_STA_AUTH_WAPI_PSK
    const wifi_auth_mode_t auth_mode = WIFI_AUTH_WAPI_PSK; 
#else
    const wifi_auth_mode_t auth_mode = WIFI_AUTH_OPEN;
#endif

#if CONFIG_WIFI_STA_WPA3_SAE_PWE_HUNT_AND_PECK
    const wifi_sae_pwe_method_t sae_pwe_method = WPA3_SAE_PWE_HUNT_AND_PECK;
#elif CONFIG_WIFI_STA_WPA3_SAE_PWE_H2E
    const wifi_sae_pwe_method_t sae_pwe_method = WPA3_SAE_PWE_HASH_TO_ELEMENT;
#elif CONFIG_WIFI_STA_WPA3_SAE_PWE_BOTH
    const wifi_sae_pwe_method_t sae_pwe_method = WPA3_SAE_PWE_BOTH;
#else
    const wifi_sae_pwe_method_t sae_pwe_method = WPA3_SAE_PWE_UNSPECIFIED;
#endif

#ifndef CONFIG_WIFI_STA_WPA3_PASSWORD_IDENTIFIER
#define CONFIG_WIFI_STA_WPA3_PASSWORD_IDENTIFIER ""
#endif

wifi_config_t wifi_config = {
        .sta = {
            .ssid = CONFIG_WIFI_STA_SSID,
            .password = CONFIG_WIFI_STA_PASSWORD,
            .threshold.authmode = auth_mode,
            .sae_pwe_h2e = sae_pwe_method,
            .sae_h2e_identifier = CONFIG_WIFI_STA_WPA3_PASSWORD_IDENTIFIER
        },
    };

    ret = esp_wifi_set_config(WIFI_IF_STA, &wifi_config);
    if(ret != ESP_OK){
        ESP_LOGE(TAG, "Failed to set WiFi STA configuration");
        return ESP_FAIL;
    }

    ret = esp_wifi_start();
    if(ret != ESP_OK){
        ESP_LOGE(TAG, "Failed to start WiFi");
        return ESP_FAIL;
    }

    ESP_LOGI(TAG, "WiFi STA initialization completed");

    return ESP_OK;
}

esp_err_t wifi_sta_stop(void) {
    esp_err_t ret;

    ret = esp_wifi_stop();
    if(ret != ESP_OK){
        ESP_LOGE(TAG, "Failed to stop WiFi");
        return ESP_FAIL;
    }
    ret = esp_event_handler_unregister(WIFI_EVENT,
                                                WIFI_EVENT_STA_START,
                                                &on_wifi_event);
    if(ret != ESP_OK){
        ESP_LOGE(TAG, "Failed to unregister WiFi start event handler");
        return ESP_FAIL;
    }

    ret = esp_event_handler_unregister(WIFI_EVENT,
                                                WIFI_EVENT_STA_STOP,
                                                &on_wifi_event);
    if(ret != ESP_OK){
        ESP_LOGE(TAG, "Failed to unregister WiFi stop event handler");
        return ESP_FAIL;
    }

    ret = esp_event_handler_unregister(WIFI_EVENT,
                                                WIFI_EVENT_STA_CONNECTED,
                                                &on_wifi_event);
    if(ret != ESP_OK){
        ESP_LOGE(TAG, "Failed to unregister WiFi connected event handler");
        return ESP_FAIL;
    }

      ret = esp_event_handler_unregister(WIFI_EVENT,
                                                WIFI_EVENT_STA_CONNECTED,
                                                &on_wifi_event);
    if(ret != ESP_OK){
        ESP_LOGE(TAG, "Failed to unregister WiFi connected event handler");
        return ESP_FAIL;
    }

     ret = esp_event_handler_unregister(WIFI_EVENT,
                                                WIFI_EVENT_STA_DISCONNECTED,
                                                &on_wifi_event);
    if(ret != ESP_OK){
        ESP_LOGE(TAG, "Failed to unregister WiFi disconnected event handler");
        return ESP_FAIL;
    }
#if CONFIG_WIFI_STA_CONNECT_IPV4 || CONFIG_WIFI_STA_CONNECT_UNSPECIFIED
    ret = esp_event_handler_unregister(IP_EVENT,
                                                IP_EVENT_STA_GOT_IP,
                                                &on_ip_event);
    if(ret != ESP_OK){
        ESP_LOGE(TAG, "Failed to unregister IP got IPv4 event handler");
        return ESP_FAIL;
    }
#endif
#if CONFIG_WIFI_STA_CONNECT_IPV6 || CONFIG_WIFI_STA_CONNECT_UNSPECIFIED
    ret = esp_event_handler_unregister(IP_EVENT,
                                                IP_EVENT_STA_GOT_IP6,
                                                &on_ip_event);
    if(ret != ESP_OK){
        ESP_LOGE(TAG, "Failed to unregister IP got IPv6 event handler");
        return ESP_FAIL;
    }
#endif
    ret = esp_event_handler_unregister(IP_EVENT,
                                                IP_EVENT_STA_LOST_IP,
                                                &on_ip_event);
    if(ret != ESP_OK){
        ESP_LOGE(TAG, "Failed to unregister IP lost IP event handler");
        return ESP_FAIL;
    }

    ret = esp_unregister_shutdown_handler((shutdown_handler_t) esp_wifi_stop);
    if(ret != ESP_ERR_INVALID_STATE){
        ESP_LOGI(TAG, "Shutdown handler already unregistr");
    }else if(ret != ESP_OK){
        ESP_LOGE(TAG, "Failed to unregister WiFi shutdown handler");
        return ESP_FAIL;
    }

    ret = esp_wifi_disconnect();
    if(ret == ESP_ERR_WIFI_NOT_STARTED || ret == ESP_ERR_WIFI_NOT_STARTED){
        ESP_LOGI(TAG, "WiFi already disconnected");
    } else if(ret != ESP_OK){
        ESP_LOGE(TAG, "Error (%d): Failed to disconnect WiFi", ret);
        return ESP_FAIL;
    }

    ret = esp_wifi_stop();
    if(ret == ESP_ERR_WIFI_NOT_INIT){
        ESP_LOGI(TAG, "WiFi already stopped");
    } else if(ret != ESP_OK){
        ESP_LOGE(TAG, "Error (%d): Failed to stop WiFi", ret);
        return ESP_FAIL;
    }

    ret = esp_wifi_deinit();
    if(ret == ESP_ERR_WIFI_NOT_INIT){
        ESP_LOGI(TAG, "WiFi already deinitialized");
    } else if(ret != ESP_OK){
        ESP_LOGE(TAG, "Error (%d): Failed to deinitialize WiFi", ret);
        return ESP_FAIL;
    }

    if(s_wifi_driver != NULL) {
        esp_wifi_destroy_if_driver(s_wifi_driver);
        s_wifi_driver = NULL;
    }
    if(s_wifi_netif!= NULL) {
        esp_netif_destroy(s_wifi_netif);
    }

    if(s_wifi_event_group != NULL) {
        xEventGroupClearBits(s_wifi_event_group, WIFI_STA_CONNECTED_BIT);
        xEventGroupClearBits(s_wifi_event_group, WIFI_STA_IPV4_OBTAINED_BIT);
        xEventGroupClearBits(s_wifi_event_group, WIFI_STA_CONNECTED_BIT);
    }

    s_wifi_netif = NULL;

    ESP_LOGI(TAG, "WiFi STA stopped");

    return ESP_OK;
}

esp_err_t wifi_sta_reconnect(void) {
    esp_err_t ret;

    ret = wifi_sta_stop();
    if(ret != ESP_OK){
        ESP_LOGE(TAG, "Failed to stop WiFi STA for reconnection");
        return ret;
    }
    ret = wifi_sta_init(NULL);
    if(ret != ESP_OK){
        ESP_LOGE(TAG, "Failed to reinitialize WiFi STA for reconnection");
        return ret;
    }

    ESP_LOGI(TAG, "WiFi STA reconnect initiated");

    return ESP_OK;
}

